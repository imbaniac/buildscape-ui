<script lang="ts">
  ...
  // Calculate island color based on TPS (transactions per second)
  // High TPS = lush green, Low TPS = desert brown/gray
  function calculateIslandColors(tps: number) {
    // Define TPS thresholds (you can adjust these based on actual data)
    const maxTps = 100; // Consider 100+ TPS as very active
    const minTps = 0.1; // Consider < 0.1 TPS as very inactive
    
    // Normalize TPS to 0-1 range (using log scale for better distribution)
    const normalizedTps = tps > 0 
      ? Math.min(1, Math.max(0, Math.log10(tps + 1) / Math.log10(maxTps + 1)))
      : 0;
    
    // Define color gradients from desert (low TPS) to lush green (high TPS)
    const desertColors = {
      main: "#8B7355",    // Sandy brown
      dark: "#6B5345",    // Dark brown
      accent: "#9B8365"   // Light brown
    };
    
    const lushColors = {
      main: "#4A7C59",    // Lush green
      dark: "#2A5C39",    // Dark green
      accent: "#5A8C69"   // Light green
    };
    
    // Interpolate between colors based on TPS
    function interpolateColor(desert: string, lush: string, t: number): string {
      const d = parseInt(desert.slice(1), 16);
      const l = parseInt(lush.slice(1), 16);
    ...
    return {
      main: interpolateColor(desertColors.main, lushColors.main, normalizedTps),
      dark: interpolateColor(desertColors.dark, lushColors.dark, normalizedTps),
      accent: interpolateColor(desertColors.accent, lushColors.accent, normalizedTps)
    };
  }
  
  const islandColors = calculateIslandColors(tps);
  const islandColor = islandColors.main;
  const islandDarkColor = islandColors.dark;
  const islandAccentColor = islandColors.accent;
  
  // Determine which gradient to use based on TPS
  function getGradientId(tps: number): string {
    if (tps < 1) return "islandGradientDesert";
    if (tps < 10) return "islandGradientSemiArid";
    if (tps < 50) return "islandGradientGrassland";
    return "islandGradientLush";
  }
  
  const gradientId = getGradientId(tps);
...
>
  <g>
    ...
    <g transform="scale({scale})" opacity="0.9">
      ...
      <!-- Overlay terrain texture (disabled on mobile for performance) -->
      {#if !isTouchDevice}
        <g transform="translate(-931, -560)" opacity="0.3">
          <path
            d="M686.595 933.906L244.381 678.593L321.962 633.802L764.176 889.114L686.595 933.906ZM1551.63 537.5L1629.21 582.291L1008.56 940.624L930.977 895.833L853.396 940.624L232.745 582.291L310.326 537.5L232.745 492.708L389.458 402.229L311.877 357.438L592.334 195.516L669.915 240.308L853.396 134.375L930.977 179.167L1008.56 134.375L1629.21 492.708L1551.63 537.5ZM1268.46 880.156L1190.87 835.364L1471.33 673.442L1548.91 718.234L1268.46 880.156ZM1539.99 441.197L1097.78 185.885L1175.36 141.093L1617.57 396.406L1539.99 441.197Z"
            fill="url(#terrainTextureSimple)"
          />
        </g>
      {/if}
    </g>
