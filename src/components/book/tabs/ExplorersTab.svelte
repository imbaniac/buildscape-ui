<script lang="ts">
  ...
  function detectExplorerType(
    url: string,
  ): "official" | "alternative" | "analytics" {
    const domain = new URL(url).hostname.toLowerCase();

    // Official explorers
    if (
      domain.includes("etherscan.io") ||
  ...
  function getExplorerStyle(name: string): {
    background: string;
    color: string;
  } {
  ...
  const alternativeExplorers = explorers.filter(
...
<div class="explorers-modern">
  {#if officialExplorers.length > 0}
    <div class="explorer-section">
      ...
      <div class="explorer-cards">
        {#each officialExplorers as explorer (explorer.name)}
  ...
  {#if alternativeExplorers.length > 0}
    <div class="explorer-section">
      ...
      <div class="explorer-cards">
        {#each alternativeExplorers as explorer (explorer.name)}
          {@const style = getExplorerStyle(explorer.name)}
  ...
  {#if analyticsExplorers.length > 0}
    <div class="explorer-section">
      ...
      <div class="explorer-cards">
        {#each analyticsExplorers as explorer (explorer.name)}
          {@const style = getExplorerStyle(explorer.name)}
          <a
            href={explorer.url}
            target="_blank"
            class="explorer-card analytics"
          >
            <div
              class="explorer-logo"
              style="background: {style.background}; color: {style.color};"
