<script lang="ts">
  ...
  const pressureStatus = $derived(() => {
    if (safeUtilization <= 30) return { text: 'LOW', color: '#10b981', class: 'low' };
    if (safeUtilization <= 60) return { text: 'MODERATE', color: '#f59e0b', class: 'moderate' };
    if (safeUtilization <= 85) return { text: 'HIGH', color: '#f97316', class: 'high' };
  ...
  <div class="panel-body">
    <div class="resources-grid">
      <div class="resource-item">
        <Tooltip text={`Network utilization: ${safeUtilization}% - ${pressureStatus().text} pressure`}>
